// Server-only helper: renderiza una plantilla de notificaciones y la inserta.
// Centraliza la sanitización, el fallback de `nombre` y una deduplicación de
// 10s para plantillas `dev_*` (evita spam si el dev hace clic varias veces).
// El nombre `.server.ts` bloquea imports desde código cliente.
